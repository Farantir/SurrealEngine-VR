...
void UPawn::Tick(float elapsed)
{
	...
	if (Role() == ROLE_Authority)
	{
		...
		// Native pawn perception tick. Pawn.uc documents SightCounter as "Used to keep
		// track of when to check player visibility" and its own SeePlayer/EnemyNotVisible
		// are declared as empty probe events - the periodic sight check that fires them
		// (and auto-updates LastSeenPos/LastSeeingPos/LastSeenTime) is the engine's job,
		// not something script polls for itself. Without this, a pawn that hasn't been
		// bumped or shot at never learns a live enemy is visible at all.
		SightCounter() -= elapsed;
		if (SightCounter() <= 0.0f)
		{
			SightCounter() += 0.2f + 0.3f * (static_cast<float>(std::rand()) / RAND_MAX);
